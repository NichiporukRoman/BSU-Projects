...
void syncTests(Matrix& a, Matrix& b){
    ...
    Matrix startResult = Matrix::matrixBlockMultiply(a, b, 2);
    auto end = std::chrono::high_resolution_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
    for (int i = 1; i <= a.getMatrix().size(); ++i) {
        start = std::chrono::high_resolution_clock::now();
        Matrix result = Matrix::matrixBlockMultiply(a, b, i);
...
int main() {
    ...
    Matrix::writeMatrix(Matrix::matrixBlockMultiply(mx1,mx2,5));
    syncTests(mx1, mx2);
}